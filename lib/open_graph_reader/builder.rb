module OpenGraphReader
  ...
  class Builder
    ...
    # no inputs for when content type is image
    ...
    def base
      ...
      if @parser.any_tags?
        ...
        # fill in og with regular head metadata
        root = 'og'
    ...
    def image_base
      ...
      object = Object::Registry["#{root}:#{name}"].new
      collection << object
      object.content = OpenGraphReader.current_origin
    ...
    def synthesize_title base
      ...
      if defined?(@parser) and !@parser.nil?
        base.og["title"] = @parser.title
      else
        base.og["title"] = OpenGraphReader.current_origin
      end
    end
