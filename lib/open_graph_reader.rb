...
module OpenGraphReader
  ...
  def self.fetch! url
    ...
    when URI
      ...
      if target.html?
        ...
        parse_image! target.url
      else
        parse_any! target.url
        # raise NoOpenGraphDataError, "#{url} doesn't contain any HTML or image" 
      end
    else
  ...
  def self.parse! html, origin=nil
    ...
    # fallback to title in head when no opengraph metadata
  ...
  def self.parse_image! origin=nil
    self.current_origin = origin
    Builder.new.image_base.tap {|image_base|
      image_base.origin = origin.to_s if origin
      self.current_origin = nil
    }
  end

  def self.parse_any! origin=nil
    self.current_origin = origin
    Builder.new.any_base.tap {|image_base|
      image_base.origin = origin.to_s if origin
      self.current_origin = nil
    }
  end
